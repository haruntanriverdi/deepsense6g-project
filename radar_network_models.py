...
class Advenced_RadarCube(nn.Module):
    def __init__(self):
        super(Advenced_RadarCube, self).__init__()
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(self.pool2(self.conv2(x)))
        x = F.leaky_relu(self.pool(self.conv3(x)))
        x = F.leaky_relu(self.pool(self.conv4(x)))
        x = F.leaky_relu(self.pool(self.conv5(x)))
