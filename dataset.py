...
def load_radar_data(root_dir, csv_file, radar_column='unit1_radar_1', label_column='beam_index_1'):
    ...
    print(os.path.abspath(root_dir + csv_file[radar_column][0][1:]))
    X = loadmat(os.path.abspath(root_dir + csv_file[radar_column][0][1:]))['data'] # Load the first data point
    X = np.zeros((len(csv_file),) + X.shape, dtype=X.dtype)
    for i in tqdm(range(len(csv_file))):
        X[i] = loadmat(os.path.abspath(root_dir +  csv_file[radar_column][i][1:]))['data']
